...
class PSFBeadsOutput(MetricsOutput):
    ...
    considered_bead_centers_intensity_std_outlier: Optional[list[Roi]] = Field(default=None, description="""The centers of the beads detected but considered as being too intense and potentially represent a cluster of beads. This is calculated using the standard deviation of the intensity. Intensity robust z-score over requested threshold. Measurements on these bead not considered for the dataset analysis. One point will be provided per bead. One ROI per channel.""", json_schema_extra = { "linkml_meta": {'domain_of': ['PSFBeadsOutput']} })
